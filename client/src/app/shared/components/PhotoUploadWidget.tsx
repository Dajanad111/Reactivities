...
export default function PhotoUploadWidget({uploadPhoto, loading}: Props) {
    ...
    const onDrop = useCallback((acceptedFiles: File[]) => {  //konfiguracija dropzone
        setFiles(acceptedFiles.map((file: object) => Object.assign(file, {
            preview: URL.createObjectURL(file as Blob)
        })))
    }, []);

    const onCrop = useCallback(() => { //konfiguracija croppera
    ...
    const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop });  //dio react dropzone
    return (
